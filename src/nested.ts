...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((all, q) => all + q.points, 0);
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions
        .filter((q) => q.published) // to only keep the public questions
        .reduce((all, q) => all + q.points, 0);
}
        ...
        q.id === targetId ?
            {
                ...
                options:
                    (
                        newQuestionType === "multiple_choice_question" // making sure it's multiple choice
                    ) ?
                        q.options
                    :   [],
            }
        ...
        q.id === targetId ?
            {
                ...
                options:
                    targetOptionIndex === -1 ?
                        [...q.options, newOption] // appending a new option
                    :   q.options.map(
                            (option, index) =>
                                index === targetOptionIndex ? newOption : (
                                    option
                                ), // replacing existing option
                        ),
            }
        :   q,
    );
